...
;を関数の宣言のあとに付け足した。
ave_onlineとval_onlineの中身をかいた。
N-1までの平均や二乗の平均をavee,square_aveeに格納した。
...
- `double N`としていますが，`N`は理論上，整数しか入りません．計算の都合でdoubleにしているのだとしたら，計算順序を変えるなどして，ここは`int N`としてください．
- `int N`とすることで他の場所で影響が出るかと思います．計算順序を改めて精度を落とさないように書き換えてください．
- `avee`, `sqare_avee`などという安易な名前はやめたほうがいいです．eをつけるだけとかだと，使い間違いそうです．きちんと意味をもった名前にしましょう．
