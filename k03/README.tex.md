...
## ソースコードの説明
sscanfで文字列を読み取り、平均、分散、乱数の数を代入する。
srand(RAND_SEED)で標準正規分布を線形移動させる式をつくり、任意の平均と分散に従う乱数生成関数を実現した。
r_stdnormも少し訂正した。
...
[comment #20200430]
- OKです．
- ちなみに，捏造した標本から母集団平均と母集団分散を推定してみると，真の170.8や5.43とどのくらいずれますか？この母集団平均の推定値は捏造するたび変わります．すなわち確率分布します．この確率分布は正規分布し，その標準偏差は標準誤差と呼ばれています．
